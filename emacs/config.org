...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-todo-keywords
        '((sequence "TODO(t!)" "|" "DONE(d!)" "REFERENCE(r!)")))
#+END_SRC
...
task can be resolved by marking it as =DONE= or =REFERENCE=. Tasks
marked as =DONE= can be safely archived, while =REFERENCE= tasks may
be relevant as a reference in the future (i.e. notes about interesting
articles, etc).
...
the daily or weekly reviews) and the agenda to identify the things I'm
going to work on next. Similarly with a =SOMEDAY= state for
