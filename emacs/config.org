...
loads the Emacs Lisp code in this file using Org-babel:
...
Keep a list of base packages so they can be easily installed with
=my-install-based-packages=. =org-plus-contrib= must be installed
separately after starting Emacs with =emacs -q=, otherwise the
Org-mode installation breaks.
...
few [[https://ifttt.com][IFTTT]] recipes that append content to that file). I keep separate
